...
하나의 지표만 맹신하지 않고 추세, 모멘텀, 변동성, 거래량, 독립 확인 점수를 계산한 뒤
자산군별 프로필로 합산합니다. ADX 추세 강도와 구성요소 충돌에 따라 점수·신뢰도를
적응형으로 보정하며, 주식과 24시간 거래되는 코인의 차이는 서로 다른 임계값으로 처리합니다.

## 주요 기능

- 공통 OHLCV 데이터 모델
- SMA, EMA, RSI, MACD, Bollinger Bands, ATR, Stochastic, OBV, 거래량 비율
- ADX/+DI/-DI, MFI, 12기간 ROC, 20기간 rolling VWAP
- 추세·모멘텀·변동성·거래량·독립 확인 5개 구성 점수
- ADX 기반 추세 가중치 조절, 가중 합의율과 구성 충돌 신뢰도 감점
...
| 구성 | 핵심 입력 | 최대 방향성 |
|---|---|---:|
| Trend | SMA 20/50, EMA 12/26, MACD histogram, ADX 적응 가중치 | ±42 |
| Momentum | RSI 14, Stochastic 14/3 | ±24 |
| Volatility | Bollinger 20/2, ATR 14 | ±12 및 고변동성 감점 |
| Volume | OBV 변화, 20기간 거래량 비율 | ±14 |
| Confirmation | ADX 방향, MFI 14, ROC 12, rolling VWAP 20 | ±12 |

최종 점수는 `-100..100` 범위로 제한됩니다. 코인은 주식보다 더 높은 매수·매도 임계값과
ATR 위험 허용치를 사용합니다. 신뢰도는 절대 점수뿐 아니라 구성요소의 가중 방향 합의율,
지표 완전성, 충돌 감점과 약한 ADX 감점을 함께 반영합니다. API 응답의 `agreement_pct`와
`conflict_penalty`에서 보정 근거를 직접 확인할 수 있습니다.
